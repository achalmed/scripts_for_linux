...
EXCLUDED_DIRS = [
    ...
    # _extensiones
    "pecunia-fluxus/_extensions/",
    "numerus-scriptum/_extensions/",
    "epsilon-y-beta/_extensions/",
    "optimums/_extensions/",
    "methodica/_extensions/",
    "chaska/_extensions/",
    "axiomata/_extensions/",
    "actus-mercator/_extensions/",
    "res-publica/_extensions/",
    "website-achalma/_extensions/",
    "dialectica-y-mercado/_extensions/",
    "borradores/tesis 2025/_extensions/",
...
def print_header(text: str, char: str = "═") -> None:
    ...
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}║{' ' * padding}{text}{' ' * (width - len(text) - padding - 2)}║{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}{'╚' + char * (width - 2) + '╝'}{Colors.ENDC}\n"
    )
...
def print_box_info(label: str, value: str, icon: str = "📋") -> None:
    ...
    print(
        f"{Colors.OKCYAN}{icon} {label}:{Colors.ENDC} {Colors.BOLD}{value}{Colors.ENDC}"
    )
...
def create_hardlinks(
    ...
    exclude_dirs = set(
        os.path.normpath(os.path.join(search_dir, d)) for d in exclude_dirs
    )
    ...
    for root, dirs, files in os.walk(search_dir, topdown=True):
        ...
        dirs[:] = [
            d
            for d in dirs
            if os.path.normpath(os.path.join(root, d)) not in exclude_dirs
        ]
    ...
    print_success(
        f"Se encontraron {stats['total_files']} archivo(s) con el nombre '{filename}'"
    )

    # Contar grupos (excluyendo grupos con un solo archivo)
    groups_with_multiple_files = [
    ...
    if stats["groups_found"] == 0:
        print_info(
            "Todos los archivos tienen contenido único, no hay candidatos para hard links"
        )
        return stats

    print_success(
        f"Se encontraron {stats['groups_found']} grupo(s) de archivos con contenido idéntico\n"
    )
    ...
    for file_hash, file_list in hash_groups.items():
        ...
        print_separator()
        print(
            f"{Colors.BOLD}{Colors.LIGHT_BLUE}🔍 GRUPO #{group_number}{Colors.ENDC} - {Colors.GRAY}Hash: {file_hash[:16]}...{Colors.ENDC}\n"
        )
        ...
        print(
            f"{Colors.OKGREEN}📌 Archivo fuente:{Colors.ENDC} {Colors.BOLD}{get_relative_path(source_path, search_dir)}{Colors.ENDC}"
        )
        print(
            f"{Colors.GRAY}   Tamaño: {size_str} | Inodo: {source_inode}{Colors.ENDC}\n"
        )
        ...
        if already_linked:
            print(
                f"{Colors.GRAY}⏭️  Archivos ya enlazados ({len(already_linked)}):{Colors.ENDC}"
            )
            for filepath in already_linked:
                print(
                    f"{Colors.GRAY}   • {get_relative_path(filepath, search_dir)}{Colors.ENDC}"
                )
            print()
        ...
        for i, filepath in enumerate(candidates, 1):
            print(
                f"{Colors.OKCYAN}   {i}. {get_relative_path(filepath, search_dir)}{Colors.ENDC}"
            )
        print()
        ...
        if not auto_mode and not dry_run:
            response = (
                input(
                    f"{Colors.BOLD}¿Crear hard links para este grupo? [S/n]: {Colors.ENDC}"
                )
                .strip()
                .lower()
            )
        ...
        if dry_run:
            ...
            for filepath in candidates:
                try:
                    ...
                    print_success(
                        f"Hard link creado: {get_relative_path(filepath, search_dir)}"
                    )
                    success_count += 1
                except OSError as e:
                    print_error(
                        f"Error al crear hard link para {get_relative_path(filepath, search_dir)}: {e}"
                    )
                    stats["errors"] += 1
...
def print_summary(stats: Dict[str, int]) -> None:
    ...
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}  {Colors.OKGREEN}✅ Grupos creados:{Colors.ENDC} {Colors.BOLD}{stats['groups_created']}{Colors.ENDC}"
        + " " * (57 - len(str(stats["groups_created"])))
        + f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}  {Colors.OKCYAN}📝 Hard links creados:{Colors.ENDC} {Colors.BOLD}{stats['links_created']}{Colors.ENDC}"
        + " " * (53 - len(str(stats["links_created"])))
        + f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}  {Colors.GRAY}⏭️  Archivos omitidos:{Colors.ENDC} {Colors.BOLD}{stats['files_skipped']}{Colors.ENDC} (ya eran hard links)"
        + " " * (33 - len(str(stats["files_skipped"])))
        + f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}  {Colors.WARNING}⚠️  Grupos omitidos:{Colors.ENDC} {Colors.BOLD}{stats['groups_skipped']}{Colors.ENDC}"
        + " " * (56 - len(str(stats["groups_skipped"])))
        + f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}"
    )
    if stats["errors"] > 0:
        print(
            f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}  {Colors.FAIL}❌ Errores:{Colors.ENDC} {Colors.BOLD}{stats['errors']}{Colors.ENDC}"
            + " " * (67 - len(str(stats["errors"])))
            + f"{Colors.BOLD}{Colors.OKBLUE}║{Colors.ENDC}"
        )
    print(f"{Colors.BOLD}{Colors.OKBLUE}╚{'═' * 78}╝{Colors.ENDC}")
    ...
    if stats["groups_created"] > 0:
        ...
        print(
            f"{Colors.GRAY}   Usa 'detect_hardlinks_tree.sh' para verificar los enlaces creados{Colors.ENDC}\n"
        )
...
{Colors.BOLD}Ejemplos de uso:{Colors.ENDC}
  ...
  python create_hardlinks.py documento.py --exclude temp build dist
    ...
    parser.add_argument(
        "--no-color", action="store_true", help="Desactivar colores en la salida"
    )
    ...
    try:
        stats = create_hardlinks(
            search_dir, args.filename, exclude_dirs, args.auto, args.dry_run
        )
        print_summary(stats)
